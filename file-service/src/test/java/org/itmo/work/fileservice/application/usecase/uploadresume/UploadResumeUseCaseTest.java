...
class UploadResumeUseCaseTest {
    ...
    @Mock FileEventPublisherPort fileEventPublisherPort;
    ...
    @InjectMocks UploadResumeUseCase useCase;
    ...
    void uploadResume_whenApplicationNotExists_shouldThrowApplicationNotFound_andDoNothingElse() {
        ...
        verify(applicationRegistryPort, never()).findApplicationRegistryByApplicationId(any());
    }
    ...
    @Test
    void uploadResume_whenOwnerMismatch_shouldThrowApplicationNotFound_andNotUploadNotSaveNotPublish() {
        UUID applicationId = UUID.randomUUID();
        ...
        ApplicationRegistry reg = mock(ApplicationRegistry.class);
        when(reg.getOwnerId()).thenReturn(ownerId);
        when(applicationRegistryPort.findApplicationRegistryByApplicationId(applicationId)).thenReturn(reg);

        when(currentUserPort.getCurrentUserId()).thenReturn(currentId);
        ...
        verify(applicationRegistryPort).exists(applicationId);
        verify(storedFileRepositoryPort).checkResumeAlreadyExists(applicationId);
        verify(applicationRegistryPort).findApplicationRegistryByApplicationId(applicationId);
        verify(reg).getOwnerId();
        verify(currentUserPort).getCurrentUserId();

        verifyNoInteractions(fileStoragePropsPort, fileStoragePort, fileEventPublisherPort);
        verify(storedFileRepositoryPort, never()).save(any());
    }
    ...
    void uploadResume_success_shouldPutSavePublishAndReturnId_andDefaultOriginalNameIfNull() {
        ...
        assertTrue(keyCap.getValue().startsWith("resume/"));
