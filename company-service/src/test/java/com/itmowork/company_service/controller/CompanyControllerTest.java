...
public class CompanyControllerTest extends AbstractIntegrationTest{
    ...
    @Test
    void shouldValidateCompanyOwnership() {
        var client = withJwt(USER_ID, "user@test.com", "ROLE_ADMIN");
        ...
        client.get()
                .uri("/api/company/{companyId}/{userId}", created.id(), USER_ID)
                .exchange()
                .expectStatus().isOk()
                .expectBody(Boolean.class)
                .isEqualTo(true);
        ...
        client.get()
                .uri("/api/company/{companyId}/{userId}",
                        created.id(),
                        UUID.randomUUID())
                .exchange()
                .expectStatus().isOk()
    ...
    @Test
    void shouldCheckCompanyExistsById() {
        var client = withJwt(USER_ID, "user@test.com", "ROLE_ADMIN");
        ...
        client.get()
                .uri("/api/company/{id}", UUID.randomUUID())
                .exchange()
                .expectStatus().isOk()
                .expectBody(Boolean.class)
                .isEqualTo(false);
